...
export function getAllCategories(categoryName?: string, categoryCode?: string) {
  return apiResolver<Response<CategoryDTOResponse[]>>(() =>
    axios.get("", {
      headers: {
...
export function getSubCategoriesByCategory(
  ...
  return apiResolver<Response<SubCategoryResponseDTO[]>>(() =>
    axios.get(`/sub-categories/${categoryId}`, {
      params: {
...
type UpdateCategoryRequestDTO = {
  ...
  desc: string;
  status: "1" | "0";
  category_code?: string; // to change its parent category
  datetime: string;
  signature: string; // message = token, Code, Date
}
